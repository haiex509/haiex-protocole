...
async function main() {
  ...
  await getBalance();
  const token = ethereum.address;

  console.log('Buying $100 of tgoud');
  const amountUsd = ethers.BigNumber.from(100).mul(ethers.BigNumber.from(10).pow(18));
  ...
  const estimateUsd = ethers.BigNumber.from(100).mul(ethers.BigNumber.from(10).pow(18));
  const estimate = await haiex.swapEstimation([usd.address, token], estimateUsd);

  console.log(estimate.toString());

  console.log('swap  tgoud token');
  const amount4 = ethers.BigNumber.from(balanceTgoud);
  const swap = await haiex.swapStable(tgoud.address, token, [usd.address, token], amount4);
  await swap.wait();
  ...
  const swap2 = await haiex.swapStable(token, tgoud.address, [token, usd.address], amount2);
  await swap2.wait();
